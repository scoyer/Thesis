        ...
        for line in fin:
            ...
            if line:
                if line.startswith('#'):
                    ...
                    triple = line.split(' ')[1:]
                    if len(triple) == 2:
                        triple = [triple[0], "is", triple[1]]
                        kb_arr.append(triple)
                    elif len(triple) == 3:
                        kb_arr.append(triple)
                    elif len(triple) == 4:
                        triple = [triple[0],  triple[1] + "_" + triple[2], triple[3]]
                        kb_arr.append(triple)
                    else:
                        pass
                else:
                    ...
                    # Get entity list
                    value_list, slot_list = generate_entity_from_kb(task_type, kb_arr, value_slot_dict)
...
def generate_entity_from_kb(task_type, kb_arr, global_entity, entity_list=None, entity_type_list=None):
    ...
    for key_value, slot, value in kb_arr:
        if task_type == "navigate":
            ...
            if key_value not in entity_list:
                entity_list.append(key_value)
                entity_type_list.append("weekly_time" if key_value == "today" else "location")
            
            if value not in entity_list:
                if "_" in slot:
                    entity_list.append(value)
                    entity_type_list.append("temperature")
                elif slot in ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]:
                    entity_list.append(value)
                    entity_type_list.append("weather_attribute")
                else:
                    assert slot == "is"
                    entity_list.append(value)
                    entity_type_list.append("date")
...
def prepare_data_seq(batch_size=100):
    ...
    for key in entities.keys():
        if key != 'poi':
            global_entities[key] = list(map(f, entities[key]))
        else:
            for item in entities['poi']:
                for k in item.keys():
                    if k == "type":
                        continue
                    if k not in global_entities:
                        global_entities[k] = []
                    global_entities[k] += [item[k].lower().replace(' ', '_')]
