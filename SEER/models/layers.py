...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        assert (mode in ["dot", "general", "mlp","align"]), (
    ...
    def forward(self, query, memory, value=None, mask=None, return_weights=False, return_weights_only=False):
        """
        ...
        if self.mode == "dot":
            ...
            assert query.size(-1) == memory.size(-1)
            # (batch_size, query_length, memory_size)
            key = self.relu(self.linear(query))
            value = self.relu(self.linear(memory))
            attn = torch.bmm(key, value.transpose(1, 2))
